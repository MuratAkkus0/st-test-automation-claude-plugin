...
- **`{PROBLEM_SUMMARY}`** — one short paragraph (1–3 sentences, never longer) describing what is wrong and what the partner must do. Pick the branch that matches the actual failure. Keep it plain and technical, no filler.
  ...
  - **Conversion payload sends null for shipping/items** → `Der Partner sendet "shipping" und "items" als Null an uns. Da der Request serverseitig ausgeführt wird, ist er im DevTools-Netzwerk-Tab nicht sichtbar. Der Partner muss den Conversion-Tag überprüfen und sicherstellen, dass die Pflichtfelder "items" und "shipping" korrekt aus dem DataLayer befüllt werden.`
...
- **`{OUTCOME_PARAGRAPH}`** — pick the variant that matches the result. Keep to 1–3 sentences each.
  ...
  - **Conversion payload sends null for shipping/items** → `The integration works end-to-end, but the Conversion Tag sends "shipping" and "items" as null. Because the request runs server-side, it is not visible in your browser DevTools — please verify in your tag configuration that "items" and "shipping" are populated from your data layer.`
