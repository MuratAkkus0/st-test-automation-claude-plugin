...
- `agents/auto-committer.md` Step 6 now spells out that `git add -A` is unconditional. The auto-committer was caught inferring from prior assistant chat ("you'll need to `git add hepler-documantations/` separately") that 6 untracked, non-gitignored documentation files should be left out of the auto-commit — silently leaving work uncommitted and forcing a manual round-trip, the exact friction the hook exists to remove. The agent now treats `.gitignore` + the Step 3 secrets scan as the only exclusion authorities and explicitly forbids selective staging or commit-splitting based on conversation interpretation. Hard-safety-constraints list updated to match. New `memory/lessons-learned/auto-committer-stages-all-changes.md` codifies the same rule for future automations.
